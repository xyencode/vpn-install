...
# These iptables rules must be inserted above all others in the INPUT chain
    ...
    echo
    GEOIP="no"
    read -p "GEOIP: Would you want to restrict access for PPTP by geographical locations? [no] " ANSGEOIP
    : ${ANSGEOIP:=$GEOIP}
    ...
    if [ "$GEOIP" == "$ANSGEOIP" ]; then
        ...
        $DIR/geoip.sh
        echo
        echo "ATTENTION: Ok, GeoIP restrictions will be applied to PPTP VPN !!!"
        ...
        case "$ANSMODE" in
            ...
            eval iptables -I INPUT -p tcp -m tcp --dport 1723 -m geoip ! --src-cc ${CC[0]} -j DROP $COMMENT
            eval iptables -I INPUT -p gre -m geoip ! --src-cc ${CC[0]} -j DROP $COMMENT
